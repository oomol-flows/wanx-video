...
type Inputs = {
    ...
    model: "wan2.6-r2v-flash" | "wan2.6-r2v" | null;
...
async function pollForResult(sessionID: string, token: string): Promise<Outputs> {
    ...
    while (Date.now() - startTime < MAX_WAIT_MS) {
        const response = await fetch(
            `https://fusion-api.oomol.com/v1/wanx-r2v-video/result/${sessionID}`,
            {
                method: "GET",
                headers: {
...
export default async function (
    ...
    const submitResponse = await fetch(
        "https://fusion-api.oomol.com/v1/wanx-r2v-video/submit",
        {
            method: "POST",
            headers: {
